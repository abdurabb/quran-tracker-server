...
const getTopStudents = async (req, res) => {
  try {
    ...
    return res
      .status(200)
      .json({ message: "Top students fetched successfully", topStudents });
  } catch (error) {
...
const getStudentReports = async (req, res) => {
  try {
    ...
    const reports = await Mark.find(matchStage)
      .skip(skip)
      .limit(limit)
      .select(
...
const updateReport = async (req, res) => {
  try {
    ...
    const student = await Student.findById(existingMark?.studentId)
    ...
    if (student?.classId?.teacher?.toString() != req.teacherId?.toString())
      return res
        .status(400)
        .json({
          message: "You are not authorized to update marks for this student",
        });

    // Find the lesson
    const lesson = await Lesson.findById(
      lessonId ? lessonId : existingMark?.lessonId,
    );
    if (!lesson) return res.status(400).json({ message: "Lesson not found" });
    if (lesson?.isDeleted == true)
      return res
        .status(400)
        .json({ message: "Currently this lesson is not available" });

    // Get lesson type name
    let lessonTypeName = "";
    if (lesson?.lessonType) {
      if (!typeCount)
        return res.status(400).json({ message: "Type Count is required" });
  ...
  try {
    ...
    if (student?.classId?.teacher?.toString() != req.teacherId?.toString())
      return res
        .status(400)
        .json({
          message: "You are not authorized to delete marks for this student",
        });
