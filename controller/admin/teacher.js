...
const addTeacher = async (req, res) => {
  try {
    ...
    const existingTeacher = await Teacher.findOne({
      $or: [{ email: new RegExp(`^${email}$`, "i") }, { phone }],
    });
    if (existingTeacher) {
      return res.status(400).json({
        message: "Teacher already exists with this email or phone number",
      });
    }
  ...
  try {
    ...
    if (phone) {
      ...
      if (existingTeacher)
        return res
          .status(400)
          .json({ message: "Teacher already exists with this Phone Number" });
      teacher.phone = phone;
    }
...
const getTeachers = async (req, res) => {
  try {
    ...
    const teachers = await Teacher.find(query)
      .skip(skip)
      .select("name email dialCode phone image")
      .sort({ createdAt: -1 })
      .limit(limit)
      .lean();
...
const assignAClass = async (req, res) => {
  try {
    ...
    if (!classDoc) {
      return res.status(400).json({ message: "Class is Missing in this Id" });
    }
    if (!teacher) {
      return res.status(400).json({ message: "Teacher is Missing in this Id" });
    }
...
const getTeacherDetails = async (req, res) => {
  try {
    ...
    return res
      .status(200)
      .json({ message: "Teacher Details Fetched", teacher: data });
  } catch (error) {
    handleError(error, res);
  }
};
